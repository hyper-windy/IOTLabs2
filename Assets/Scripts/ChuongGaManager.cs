...
namespace ChuongGa
{
    public class ChuongGaManager : MonoBehaviour
    {
        ...
        [SerializeField]
        private ControlToggle togLed;
        [SerializeField]
        private ControlToggle togPump;

        [SerializeField]
        private Speedometer gauge1;

        [SerializeField]
        private Speedometer gauge2;
        ...
        public void Update_Status(Status_Data _status_data)
        {
            temperature.text = _status_data.temperature.ToString() + "°C";
            humidity.text = _status_data.humidity.ToString() + "%";
            gauge1.Received(_status_data.temperature);
            gauge2.Received(_status_data.humidity);
        ...
        public void Update_Button(Button_Data btn_data)
        {
            bool val = (btn_data.status == "ON");
            if (btn_data.device == "LED")
                togLed.Recived(val);
            else
                togPump.Recived(val);
        }
